...
public class Matrice {
	...
	private void creaMatrice() {
		...
		for (int i = 0; i < this.dimension; i++) {
			for (int j = 0; j < this.dimension; j++) {
				System.out.println("Saisir la valeur pour les coordonn�es (" + (i + 1) + " ; " + (j + 1) + ") :");
				try {
	...
	private void demanderDimension() {
		...
		try {
			...
			if (this.dimension <= 0) {
				throw new IllegalArgumentException("Vous devez saisir un nombre entier positif sup�rieur � 0.");
			}
		} catch (IllegalArgumentException e) {
